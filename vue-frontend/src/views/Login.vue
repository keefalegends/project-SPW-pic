...
import { Store, LogIn, Loader2 } from 'lucide-vue-next'
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (error.response && error.response.status === 401) {
      ...
      errorMsg.value = 'Terjadi kesalahan pada server.'
    }
    ...
    <div class="max-w-md w-full bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
      ...
      <div class="bg-blue-600 px-8 py-10 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
          ...
          <p class="text-blue-100 text-sm">Masuk ke panel admin</p>
        </div>
      </div>
      ...
      <div class="p-8">
        <form @submit.prevent="handleLogin" class="space-y-5">
          ...
          >
            ...
            <span>{{ loading ? 'Memproses...' : 'Login Sekarang' }}</span>
          </button>
        </form>

        <div class="mt-6 text-center">
          <router-link to="/register" class="text-sm text-slate-500 hover:text-blue-600 font-medium transition-colors">Belum punya akun? Register</router-link>
        </div>
      </div>
    </div>
  </div>
